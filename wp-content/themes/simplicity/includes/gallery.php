...
*/
		...
		if ( ( get_option('woo_resize') == "true" && $use_timthumb == "true" ) || ( $src[1] > 540 ) )
  			$photo_output .= '<div><a href="'. $src[0] .'" rel="lightbox[portfolio]" title="'.$attachment->post_excerpt.'">'.woo_image('src=' . $src[0] . '&key=image&width=' . $width_setting . '&noheight=true&link=img&return=false&meta=' . $alt . '&class=single-photo').'</a></div>';
		else
  			$photo_output .= '<div' . $style . '><a href="'. $src[0] .'" rel="lightbox[portfolio]" title="'.$attachment->post_excerpt.'"><img src="'. $src[0] .'" class="single-photo" alt="'.$alt.'" /></a>'.$caption.'</div>'; 
		
		// Save thumbnail
		$src = wp_get_attachment_image_src($att_id, $thumb_size, true);
		$thumb_output .= '<li><a href="#"><img src="'. $src[0] .'" height="'.$thumb_height.'" width="'.$thumb_width.'" class="single-thumb" alt="'.$alt.'" />' . "</a></li>\n"; 
	}  
endif; ?>

...
$options[] = array( 'name' => __( 'Display WooSlider', 'woothemes' ),
					'desc' => sprintf( __( 'Display images attached to the business page in a slider above the page content? Requires %sWooSlider%s plugin.', 'woothemes' ), '<a href="http://www.woothemes.com/products/wooslider/" title="' . __( 'Purchase WooSlider from WooThemes.com', 'woothemes' ) . '" target="_blank">', '</a>' ),
...
/* Homepage */
    ...
    $options[] = array(
                        'name'      => __( 'Homepage image format', 'woothemes' ),
                        'desc'      => __( 'Scaled images will retain their aspect ratio in respect to the browser height. Full screen images will stretch to fill the browser window - this format is not recommended for portait images. ', 'woothemes' ),
                        'id'        => $shortname.'_homepage_image_format',
                        'type'      => 'select2',
                        'options'   => array(
                                        'fullscreen'    => __( 'Full Screen', 'woothemes' ),
                                        'scaled'        => __( 'Scaled', 'woothemes' ),
                                    ),
                        'std'       => 'scaled',
                        );
...
$options[] = array( 'name' => __( 'Thumbnail Image Dimensions', 'woothemes' ),
    				'desc' => __( 'Enter an integer value i.e. 250 for the desired size which will be used when dynamically creating the images. Images will scale in height.', 'woothemes' ),
...
$options[] = array( 'name' => __( 'Single Post - Thumbnail Dimensions', 'woothemes' ),
    				'desc' => __( 'Enter an integer value i.e. 250 for the image size. Images will scale in height.', 'woothemes' ),
    				'id' => $shortname . '_image_dimensions',
    				'std' => '',
    				'type' => array(
...
if ( ( get_post_type() == 'post') || ( !get_post_type() ) ) {
    ...
    $woo_metaboxes[] = array (  'name' => '_background',
                            'std' => 'normal',
                            'label' => __( 'Background Image', 'woothemes' ),
                            'type' => 'upload',
                            'desc' => __( 'Choose a unique background for this specific post/page. The image will be stretched to fit the browser window to be sure to upload a large image.', 'woothemes' ),
                        );
...
if ( ( get_post_type() == 'post' || get_post_type() == 'product' ) || ( !get_post_type() ) ) {

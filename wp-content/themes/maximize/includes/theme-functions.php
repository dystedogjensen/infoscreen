...
?>
	...
	<aside id="connect" class="fix">
		<h2><?php if ( $title ) echo apply_filters( 'widget_title', $title ); else _e('Subscribe','woothemes'); ?></h2>
	...
	<div id="related-posts">
	    <h2><?php _e( 'Related Posts', 'woothemes' ); ?></h2>
...
function woo_social_icons() {
    ...
    <div class="social">
    	...
    	<?php if ( apply_filters( 'maximize_navigation_connect_header', true ) ) : ?>
...
function woo_featured_gallery() {
	$pattern = get_shortcode_regex();

	if ( preg_match( "/$pattern/s", get_the_content(), $match ) && 'gallery' == $match[2] ) {
		add_filter( 'shortcode_atts_gallery', 'woo_gallery_atts' );
		echo do_shortcode_tag( $match );
	}
}

function woo_gallery_atts( $atts ) {
	$atts['size'] = 'medium';
	return $atts;
}
